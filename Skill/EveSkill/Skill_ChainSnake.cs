...
public class Skill_ChainSnake : SkillActive
{
    ...
    public bool isTriggerBoom = false;
    ...
    private void Rotate()
    {
        ...
        if (Quaternion.FromToRotation(Vector3.up, dir).eulerAngles.z > 180)
            objShoot.GetComponent<SpriteRenderer>().flipX = true;
        else
            objShoot.GetComponent<SpriteRenderer>().flipX = false;
        transform.eulerAngles += new Vector3(0, 0, -180);
    }
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (isTriggerBoom == false)
            return;
