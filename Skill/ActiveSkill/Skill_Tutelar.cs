...
public class Skill_Tutelar : SkillActive
{
    ...
    public override void Init(PlayerSkillManager.SkillData _skillData, int _multiple = 1, int indexMultiple = 0)
    {
        ...
        var list = InGameManager.instance.manager_playerSkill.listTutelarCount;
        List<Transform> listCount = new List<Transform>();
        foreach (var item in list)
        {
            if (item.listNumber.Count == _multiple)
                listCount = item.listNumber;
        }
        transform.SetParent(listCount[indexMultiple]);
        transform.localPosition = Vector3.zero;
        transform.SetParent(InGameManager.instance.manager_playerSkill.objTutelar.transform);
